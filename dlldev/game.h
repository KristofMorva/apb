...
//	vec3_t	angles2;
	...
	clipHandle_t	modelindex;		//0x88
	int		modelindex2;		//*??
	int		clientNum;		//?? 0 to (MAX_CLIENTS - 1), for players and corpses
	int		frame;			//??

	int		solid;			// for client side prediction, trap_linkentity sets this properly	0x98
/*
...
typedef struct playerState_s {
	int			commandTime;	// Unsure.. 0x00 cmd->serverTime of last executed command
	int			ImTooLazy[6];
	vec3_t			origin;
	vec3_t			velocity;
	int			ImTooLazy3[9];
...
*/
	int			eFlags;			//0xb0 copied to entityState_t->eFlags
...
*/
	...
	int			weapon2;		//0xe0
	int			unknownint;
	int			weapon;			//0xe8 copied to entityState_t->weapon
	int			weaponstate;		//0xec
...
struct gclient_s {
	...
	qboolean readyToExit;           //0x3074 wishes to leave the intermission
