...
typedef void (__stdcall *tSys_Error_real)( const char *error);
...
typedef int (__stdcall *tMSG_ReadBits)( msg_t *msg, int numBits);
tMSG_ReadBits MSG_ReadBits = (tMSG_ReadBits)(0x8130730);


typedef void (__stdcall *tMSG_WriteDeltaClient)(snapshotInfo_t* , msg_t* , int , clientState_s* , clientState_s*, int);
tMSG_WriteDeltaClient MSG_WriteDeltaClient = (tMSG_WriteDeltaClient)(0x813f49c);

//typedef void (__stdcall *tMSG_WriteDeltaEntity)(snapshotInfo_t* , msg_t* , int , entityState_t* , entityState_t*, int);
//tMSG_WriteDeltaEntity MSG_WriteDeltaEntity = (tMSG_WriteDeltaEntity)(0x813eefe);

typedef void (__stdcall *tMSG_WriteDeltaPlayerstate)(snapshotInfo_t* , msg_t* , int , playerState_t* , playerState_t*);
tMSG_WriteDeltaPlayerstate MSG_WriteDeltaPlayerstate = (tMSG_WriteDeltaPlayerstate)(0x813f5a6);

typedef void (__stdcall *tMSG_WriteEntityIndex)(snapshotInfo_t*, msg_t*, int, int);
tMSG_WriteEntityIndex MSG_WriteEntityIndex = (tMSG_WriteEntityIndex)(0x813de54);

typedef void (__stdcall *tMSG_WriteBit1)(msg_t*);
tMSG_WriteBit1 MSG_WriteBit1 = (tMSG_WriteBit1)(0x81306dc);

typedef void (__stdcall *tMSG_WriteBits)(msg_t*, int bits, int bitcount);
tMSG_WriteBits MSG_WriteBits = (tMSG_WriteBits)(0x813061c);

typedef void (__stdcall *tMSG_ReadDeltaUsercmdKey)( msg_t *msg, int key, usercmd_t *from, usercmd_t *to );
tMSG_ReadDeltaUsercmdKey MSG_ReadDeltaUsercmdKey = (tMSG_ReadDeltaUsercmdKey)(0x81392ae);

typedef void (__stdcall *tMSG_SetDefaultUserCmd)( playerState_t *ps, usercmd_t *ucmd );
tMSG_SetDefaultUserCmd MSG_SetDefaultUserCmd = (tMSG_SetDefaultUserCmd)(0x8130ad0);

//typedef int (__stdcall *tMSG_ReadBitsCompress)( const byte *datasrc, byte *buffdest, int bytecount);
//tMSG_ReadBitsCompress MSG_ReadBitsCompress = (tMSG_ReadBitsCompress)(0x8130edc);

typedef int (__stdcall *tMSG_WriteBitsCompress)( char dummy, const byte *datasrc, byte *buffdest, int bytecount);
tMSG_WriteBitsCompress MSG_WriteBitsCompress = (tMSG_WriteBitsCompress)(0x8130f3c);

typedef void (__stdcall *tSV_ClientThink)( client_t *cl, usercmd_t * );
...
typedef int (__stdcall *tDB_GetXAssetTypeSize)(int type);
...
typedef const char* (__stdcall *tFS_LoadedIwdPureChecksums)(void);
...
typedef void (__stdcall *tSV_AddServerCommand_old)(client_t *client, int unkownzeroorone, const char *);
...
typedef void (__stdcall *tSV_FreeClient)(client_t* drop);
...
typedef void (__stdcall *tClientCommand)( int );
tClientCommand ClientCommand = (tClientCommand)(0x80b070c);

typedef void (__stdcall *tClientUserinfoChanged)( int );
...
//typedef void (__stdcall *tSV_SendClientGameState)(client_t *cl);
//tSV_SendClientGameState SV_SendClientGameState = (tSV_SendClientGameState)(0x816e1ba);

/*typedef void (__stdcall *tSV_ClientEnterWorld)(client_t *cl, usercmd_t* ucmd);
...
typedef void (__stdcall *tSV_LinkEntity)(gentity_t*);
tSV_LinkEntity SV_LinkEntity = (tSV_LinkEntity)(0x817eb20);

typedef void (__stdcall *tSV_UnlinkEntity)(gentity_t*);
...
typedef qboolean (__stdcall *tBG_IsWeaponValid)( playerState_t *ps, unsigned int index);
...
typedef void (__stdcall *tClientBegin)( int clientNum );
tClientBegin ClientBegin = (tClientBegin)(0x80a7700);

typedef void (__stdcall *tMSG_WriteReliableCommandToBuffer)( const char* source, char* destination,int length);
tMSG_WriteReliableCommandToBuffer MSG_WriteReliableCommandToBuffer = (tMSG_WriteReliableCommandToBuffer)(0x813e162);

typedef qboolean (__stdcall *tSEH_StringEd_GetString)( const char* input );
tSEH_StringEd_GetString SEH_StringEd_GetString = (tSEH_StringEd_GetString)(0x817fbe0);


typedef qboolean (__stdcall *tCM_TraceBox)(const float*, const float*, const float*, float);
tCM_TraceBox CM_TraceBox = (tCM_TraceBox)(0x811a5a2);

typedef clipHandle_t (__stdcall *tCM_TempBoxModel)(const float* mins, const float* maxs, int capsule);
tCM_TempBoxModel CM_TempBoxModel = (tCM_TempBoxModel)(0x811c442);

typedef void (__stdcall *tCM_TransformedBoxTrace)(trace_t* trace, const float* start, const float* end, const float* mins, const float* maxs,
...
typedef void (__stdcall *tSV_GetConfigString)( int, char*, int );
tSV_GetConfigString SV_GetConfigString = (tSV_GetConfigString)(0x8173f64);




/*
typedef void (__stdcall *tHuff_offsetReceive)( node_t *node, int *ch, byte *fin, int *offset );
